    ...
    public partial class IFunction
    {
            ...
            if (!string.IsNullOrEmpty(action))
            {
                ...
                switch (action)
                {
                    ...
                    case "SupplierInfo":
                        ...
                        break;
                    case "CheckRUsersMachine":
                        HttpContext.Current.Response.Write(CheckRUsersMachine());
                        break;
        ...
        static public Object GetUsersList()
        {
            ...
            try
            {
                ...
                SupplierInfoBLL sIBLL = new SupplierInfoBLL();
                foreach (DataRow dr in dt.Rows)
                {
                    try
                    {
                        if (!string.IsNullOrEmpty(dr["SId"].ToString()) && dr["SId"].ToString() != "0")
                        {
                            dr["SId"] = sIBLL.GetSignName(int.Parse(dr["SId"].ToString()));
                        }
